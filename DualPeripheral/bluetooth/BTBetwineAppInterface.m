...
-(BOOL)sendSetSystemTime:(NSInteger)time BeginTime:(NSInteger)beginTime EndTime:(NSInteger)endTime
{
    ...
    NSInteger sysHour = (time / 100) % 24;
    NSInteger sysMin = (time % 100) % 60;
...
- (void)stepUpdate:(Byte *)stepValue
{
    ...
    if (self.steps != oldsteps) { // steps update
        [self postNotification:CMBD_EVT_RECEIVE_STEPS userInfo:@{CMBD_NTF_DICT_KEY_DEVICE_ID:self.bleDevice.deviceId, CMBD_NTF_BW_DICT_KEY_STEPS:[NSNumber numberWithInteger:self.steps]}];
    }
    ...
    if (self.steps < self.activeMoveSteps || self.activeMoveSteps == 0 ||
        [BTCommonUtil differenceFromDate:now toDate:self.activeMoveDate] > CMBD_ACTIVE_MOVE_TIME_THRESHOLD * 3) {
        // initiate state
        ...
        if ([BTCommonUtil differenceFromDate:now toDate:oldTime] <= CMBD_ACTIVE_MOVE_TIME_THRESHOLD) {
            // confirm active move
            [self postNotification:CMBD_EVT_RECEIVE_ACTIVE_MOVE userInfo:@{CMBD_NTF_DICT_KEY_DEVICE_ID:self.bleDevice.deviceId}];
        }
...
- (void)stateUpdate:(Byte)stepState
{
    ...
    [self postNotification:CMBD_EVT_RECEIVE_ACT userInfo:@{CMBD_NTF_DICT_KEY_DEVICE_ID:self.bleDevice.deviceId, CMBD_NTF_BW_DICT_KEY_ACTIVITY:[NSNumber numberWithInteger:self.activity]}];
...
- (void)hpUpdate:(Byte)hpValue
{
    ...
    [self postNotification:CMBD_EVT_RECEIVE_ENERGY userInfo:@{CMBD_NTF_DICT_KEY_DEVICE_ID:self.bleDevice.deviceId, CMBD_NTF_BW_DICT_KEY_ENERGY:[NSNumber numberWithInteger:self.energy]}];
...
- (void)oldstepsUpdate:(Byte *)oldstepValues
{
    ...
    [self postNotification:CMBD_EVT_RECEIVE_HISTORY_STEPS userInfo:@{CMBD_NTF_DICT_KEY_DEVICE_ID:self.bleDevice.deviceId}];
}
...
-(void)battUpdate:(Byte)battValue
{
    ...
    [self postNotification:CMBD_EVT_RECEIVE_BATTERY userInfo:@{CMBD_NTF_DICT_KEY_DEVICE_ID:self.bleDevice.deviceId}];
}
...
-(void)timeUpdate:(Byte *)timeValue
{
    ...
    [self postNotification:CMBD_EVT_RECEIVE_TIME userInfo:@{CMBD_NTF_DICT_KEY_DEVICE_ID:self.bleDevice.deviceId}];
}
...
-(void)deviceInfoUpdate:(Byte *)bytes
{
    ...
    [self postNotification:CMBD_EVT_RECEIVE_DEVICE_INFO userInfo:@{CMBD_NTF_DICT_KEY_DEVICE_ID:self.bleDevice.deviceId}];
}
...
-(void)vibrateTestUpdate:(Byte *)testValues
{
    ...
    [self postNotification:CMBD_EVT_RECEIVE_LAST_VIBRATE userInfo:@{CMBD_NTF_DICT_KEY_DEVICE_ID:self.bleDevice.deviceId}];
}
...
-(void)checkSyncStateUpdate
{
    if(self.rfSyncState) {
        ...
        if(self.rfStep && self.rfActivity && self.rfEnergy) {
            ...
            [self postNotification:CMBD_EVT_RECEIVE_ALL_STATUS userInfo:@{CMBD_NTF_DICT_KEY_DEVICE_ID:self.bleDevice.deviceId}];
